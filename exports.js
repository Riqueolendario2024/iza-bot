...
const { extractMetadata } = require("./arquivos/funcoes/extractMetadata.js");
...
const {
  ...
  supre,
  recognize,
} = require("./arquivos/funcoes/functions.js"); // É necessário para algumas funções funcionar perfeitamente.
...
const joguinhodavelhajs = JSON.parse(
  fs.readFileSync("./database/usuarios/joguinhodavelha.json")
);
const aluguel = JSON.parse(
  fs.readFileSync("./database/grupos/aluguel/aluguel.json")
);
const grupos = JSON.parse(
  fs.readFileSync("./database/grupos/aluguel/grupos.json")
);
const chaves = JSON.parse(
  fs.readFileSync("./database/grupos/aluguel/chaves.json")
);
const joguinhodavelhajs2 = JSON.parse(
  fs.readFileSync("./database/usuarios/joguinhodavelha2.json")
);
...
const Limit_CMD = JSON.parse(
  fs.readFileSync("./database/func/limitarcmd.json")
);
...
const getName = (number) => {
  ps = number.includes(`@s.whatsapp.net`)
    ? number
    : number.replace(new RegExp("[()+-/ +/]", "gi"), "") + `@s.whatsapp.net`;
  returnPush = JSON.stringify(pushnames).includes(ps)
    ? pushnames[pushnames.map((i) => i.id).indexOf(ps)].nick
    : "usuário(a)";
...
const shuffle = (palavraOriginal) => {
  ...
  for (i = 0; i < palavra.length; i++) {
    armax.push({ l: palavra.split(palavra.slice(i + 1))[0].slice(i) });
  }
  shuffleProcess = "";
  ...
  for (a = 0; a < total_armax; a++) {
    ...
    armax.splice(
      toDoRandom,
      1
    ); /* Apagar o registro da palavra digitada anteriormente */
  }
...
const enviarfiguUrl = async (nagatoro, from, link, mr) => {
  ...
  exec(
    ...
    (err) => {
      ...
      nagatoro
        .sendMessage(from, { sticker: buff }, { quoted: mr })
        .catch(() => {
...
const formatNumber = (number) => {
  if (number >= 1e9)
    ...
    ); /* Retorna a quantidade na faixa dos bilhões, então já tá claro. */
  if (number >= 1e6)
    return (
      (number / 1e6).toFixed(1) + "M"
    ); /*Retorna a quantidade na casa dos milhões. */
  if (number >= 1e5)
    return (
      (number / 1e3).toFixed(1) + "K"
    ); /* Retorna a quantidade na casa acima de 100mil até 999.9mil. */
  if (number >= 1e4)
    return (
      (number / 1e3).toFixed(1) + "K"
    ); /* Retorna a quantidade na casa acima de 10mil até 99.9mil. */
  if (number >= 1e3)
    return (
      (number / 1e3).toFixed(1) + "K"
    ); /* Retorna a quantidade na casa acima de mil até 9.9mil. */
  return number;
};
...
function fuzzySimilarity(word1, word2) {
  ...
  const similarity = Math.round(
    ((2 * commonNGrams.length) / (nGrams1.length + nGrams2.length)) * 100
  );
...
function extractStateFromNumber(phoneNumber) {
  ...
  if (
    (numericOnly.startsWith(55) && numericOnly.length === 12) ||
    numericOnly.length === 13
  ) {
    const ddd = numericOnly.substring(2, 4);
    return (
      extractStateFromDDD(ddd) ||
      "Números de fora do Brasil, os estados não é possível identificar."
    );
  } else {
