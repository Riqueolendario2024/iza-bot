exports.ErrorBaileys_401 = () => {
  response = [
    "Houve uma desconexão, re-escaneie o QR, se preciso... Estou reiniciando!",
    "Minha sessão desconectou, pode reconectar e escanear o QR novamente? Gerando qr-code!",
    "Você me desconectou? A sessão precisa ser escaneada novamente... Estou reiniciando!",
  ];
...
exports.ErrorBaileys_408 = () => {
  response = [
    "A sessão sofreu um timeout, recarregando...",
    "O tempo de resposta esgotou, recarregando...",
    "Recarregando a sessão por conta de um timeout...",
  ];
  return response[Math.floor(Math.random() * response.length)];
};
...
exports.ErrorBaileys_411 = () => {
  response = [
    "A sessão salva não bate com a conexão salva, reiniciando sua sessão...",
    "O arquivo de sessão parece incorreto, estou tentando recarregar...",
    "Falha ao conectar, pois a sessão parece incorreta, recarregando...",
  ];
  return response[Math.floor(Math.random() * response.length)];
};
...
exports.ErrorBaileys_428 = () => {
  response = [
    "A conexão fechou, sua internet pode ter caído? Tentando reconectar...",
    "Não foi possível manter a conexão com o WhatsApp, tentando de novo...",
    "Sua conexão com o WhatsApp parece ter caído, reconectando...",
  ];
  return response[Math.floor(Math.random() * response.length)];
};
...
exports.ErrorBaileys_440 = () => {
  response = [
    "Outra sessão do WhatsApp Web foi aberta no meu número, feche-a...",
    "Você possui outra sessão do WhatsApp no meu número? Fecha para usar...",
    "Existem muitas sessões do WhatsApp conectadas no meu número, feche-as...",
  ];
  return response[Math.floor(Math.random() * response.length)];
};
...
exports.ErrorBaileys_500 = () => {
  response = [
    "A sessão parece mal configurada, estarei tentando reconectar...",
    "Uma reconexão será feita, pois a sessão foi considerada incorreta...",
    "A sessão parece incorreta, a conexão estará sendo reinicializada...",
  ];
  return response[Math.floor(Math.random() * response.length)];
};
...
exports.ErrorBaileys_515 = () => {
  response = [
    "Estou reiniciando, pois a sessão pediu que fosse atualizada...",
    "O sistema pediu para reiniciar, estou fazendo isso automaticamente...",
    "Meu código será reinicializado para estabilizar a conexão...",
  ];
  return response[Math.floor(Math.random() * response.length)];
};
...
exports.open = () => {
  response = [
    "Sessão aberta com o WhatsApp-Web com sucesso! Próximos logs abaixo:",
    "Conectado(a) com sucesso ao WhatsApp-Web! Próximos logs abaixo:",
    "Sessão carregada com sucesso senhor(a), próximos logs abaixo:",
    "Online, conexão estabilizada com sucesso senhor(a), próximo logs abaixo:",
  ];
  return response[Math.floor(Math.random() * response.length)];
};
...
exports.connecting = () => {
  response = [
    "Inicializando ou reiniciando meu sistema, aguarde mestre...",
    "Só um segundinho, estou carregando as informações nescessárias.",
    "Inicialização do bot está em andamento! Carregando informações, aguarde...",
    "Já tomou um copo de água hoje? Se não vai lá! Iniciando bot...",
    "Oiiieee fofuxo, tudo baum? Já tô iniciando gostoso, calma aí...",
  ];
  return response[Math.floor(Math.random() * response.length)];
};
...
exports.blackList = (GroupMetadata_, sab2) => {
  ...
  response = [
    `*@${participant}* foi removido do *${GroupMetadata_.subject}* por estar na lista negra...`,
    `Olha quem deu as caras por aqui! *@${participant}* achava que eu não ia te ver de novo bobinho(a)? 💅🏻`,
    `*@${participant}* achou que ia passar despercebido? Achou errado! Bye bye... 😵‍💫`,
    `😨 Oh my god! *@${participant}* acaba de ser banido do grupo, por estar na lista negra.`,
  ];
  return response[Math.floor(Math.random() * response.length)];
};
...
exports.phrasesLeft = (getName, sab2, encodeURIComponent) => {
  response = [
    "Devemos brindar pela partida dele(a)?",
    "Menos um nessa indústria vital...",
    "Nada pra ver aqui, ele saiu por 'acidente'...",
    `${encodeURIComponent(getName(sab2.participants[0]))} saiu do grupo...`,
  ];
  return response[Math.floor(Math.random() * response.length)];
};
...
exports.phrasesWelcome = (mdata_2, getName, sab2, encodeURIComponent) => {
  response = [
    ...
    `O ${encodeURIComponent(getName(sab2.participants[0].split("@")[0]))} acaba de cair de paraquedas aqui no grupo...`,
    "Leia as regras e divirta-se!",
    "Mais um nessa indústria vital...",
    "Gostaria de um chá enquanto lê as regras?",
    "Saudações membro novo, aqui é a gerente do estabelecimento!",
  ];
  return response[Math.floor(Math.random() * response.length)];
};
...
exports.rastrearEncomenda = (dataResult, q) => {
  return (
    ...
    dataResult.resultado
      .map(
        (info, index) =>
          `${index + 1}. *${info.description}*\n• Postagem: *${info.timeCount} (${info.datePost})*`
      )
      .join("\n–\n")
  );
};
